...
class Item:
    ...
    def __init__(self, id,name,typeof, WHD, weight, level, loadbear, updown, color):
        ''' '''
        self.id = id
        self.name = name
        self.typeof = typeof
    ...
    def string(self):
        ...
        return "%s(%sx%sx%s, weight: %s) pos(%s) rt(%s) vol(%s)" % (
            self.id, self.width, self.height, self.depth, self.weight,
...
class Bin:
    ...
    def __init__(self, id, WHD, max_weight,corner=0,put_type=1):
        ''' '''
        self.id = id
        self.width = WHD[0]
        self.height = WHD[1]
    ...
    def string(self):
        ...
        return "%s(%sx%sx%s, max_weight:%s) vol(%s)" % (
            self.id, self.width, self.height, self.depth, self.max_weight,
            self.getVolume()
        )
...
class Painter:
    ...
    def plotBoxAndItems(self,title="",alpha=0.2,write_num=False,fontsize=10):
        ...
        for item in self.items:
            ...
            color = item.color
            text= item.id if write_num else ""
